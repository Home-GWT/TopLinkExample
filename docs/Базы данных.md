...
######Разберем аннотации:
---
	<mvc:annotation-driven/> — Have Spring servlet handle all static content
	<context:annotation-config/> — Activates various annotations to be detected in bean classes: Spring's @Required and @Autowired and so on
	<tx:annotation-driven transaction-manager="transactionManager"/> — @Transaction annotation support
