...
`АОП` состоит из:
1. Аспекта (`Aspect`) - это набор интерфейсов которые обеспечивают сквозную функциональность
2. Совета (`Advice`) - это те действия (набор действий) которые должны быть выполнены до / после вызова метода (существует 5-типов советов)
