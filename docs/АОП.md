...
Языки поддерживающие концепцию АОП, реализуют следующие средства для выделения сквозной функциональности:
1. aspect (аспект) – изменяет поведение остального кода, применяя совет в точках соединения, определённых некоторым срезом;
2. advice (совет) – (дополнительная логика) код должен быть вызван из точки соединения;
3. join point (точка соединения) - точка в выполняемой программе (вызов метода, создание объекта, обращение к переменной) где следует применить совет;
4. pointcut (срез) - набор точек соединения, определяет подходит ли данная точка соединения к заданному совету;
5. introduction (внедрение) - изменение структуры класса и/или изменение иерархии наследования для добавления функциональности аспекта в инородный код;
6. target (цель) – объект к которому будут применяться советы;
7. weaving (переплетение) – связывание объектов с соответствующими аспектами (возможно на этапе компиляции, загрузки или выполнения программы).
...
Примере - предположим обычный вариант взаимодействия двух объектов, когда объект класса Caller вызывает метод operationA объекта класса Callee:
