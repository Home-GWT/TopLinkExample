...
4. Любые поля содержащие ссылки на неизменяемые объекты (например массивы, изменяемые классы типа Date...):
   1. должны быть приватными
   2. никогда не возвращаются и никаким другим образом не становятся доступными для вызыва
   3. должны быть единственными ссылками на те объекты на которые они ссылаются
   4. и не изменяют после конструирования состояние объектов на которые они ссылаются
