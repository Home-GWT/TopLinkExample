 ...
 *    --------------------------
 *    Ответы на вопросы:        
 *    - Что произойдет если в 'HashMap' метод hashCode() будет переопределен так что всегда возвращает постоянное значение а метод equlas() возращает разное значение в зависимости от класса ?
 *    - Что произойдет если в 'HashSet' метод hashCode() будет переопределен так что всегда возвращает разное значение в зависимости от класса а метод equlas() будет НЕпереопределен ?
 *    И HashMap и HashSet имеют одинаковый внутренний механизм с но только тем отличием что HashSet НЕхранит дублирующих объектов, поэтому
 *    = если присутствуют дублирующие объекты, тогда в HashSet это будет всегда только первый элемент, а в HashMap это будет всегда только последний элемент
 *    = даже если метод hashCode() будет переопределен так что всегда возвращает постоянное значение, остается метод equlas() по которому производиться основной поиск/сравнение объектов - при этом эфективность скорости доступа к объекту который исчем снизиться как если-бы это будет обычный список (LinkedList)
 *    = если присутствует НЕопределен хотябы один/любой метод hashCode(), equlas() - тогда будет утечка памяти, то есть потеряем ссылку в списке на объект
 *    --------------------------
 ...
 *                                     (JAX-WS) https://ru.wikipedia.org/wiki/JAX-WS
 * > Преимущества JAX-WS
 ...
 *   - Прямая интеграция с JAXB 2.0.Внедрение ресурсов (Resource injection).
 ...
 *  * * * * * * * *
 * > В Java имя класса ассоциируется с именем файла.
 *   Такой класс является классом верхнего уровня, то есть, это значить что этот класс будет доступен вне-пакета (... 'public class Xxx {}')
 *   Внутри файла, рядом могут быть определены много и других классов НО один из этих классов должен ассоциироваться с именем файла и иметь модификатор 'public' - по умолчанию, в Java, такой класс является классом верхнего уровня...
 *   - Если-же требуется внутри файла определить (+ объявить) уже другой класс верхнего уровня - тогда такому классу назначают модификатор 'static'
 *   - Обычно такие классы верхнего уровня определяются внутри тела другого класса (доступ к которым возможен прямо из класса...)
 * > Также могут быть и методы верхнего уровня, то есть, это значит что такой метод можна вызвать прямо из класса (без создания эземпляра этого класса...) - таким методам назначают модификатор 'static'
 *   - Обычно такие методы верхнего уровня определяются в классе который определен внутри файла.
 * - Если-же внутри тела класса определить обычный внутренний класс в котором будут 'static' методы: тогда это приведет к ошибке компиляции, потому-что если сам класс НЕявляется верхнего уровня тогда его методы тоже НЕмогут быть вызваны в верхнем уровне (без создания экземпляра класса...)
 * + Что можно было вызывать 'static' методы внутреннего класса (без создания экземпляра другого-верхнего класса) для этого нужно чтобы и сам класс тоже был 'static' 
 * ****************
...
//	}
	...
	static class DemoClass4 { // можно декларировать статические классы + можно декларировать локальные статические поля и методы
