 ...
 * > EntityManager - это интерфейс который описывает API для всех основных операций над Entity (persist|merge|remove|refresh||detach|lock).
 ...
 *             1. Transient - никогда не сохранялись и не ассоциированы ни с одной сессией;
 ...
 *      2. кэш фабрики сессий (кэш второго второго уровня) - является общим для всего приложения и используется чтобы уменьшить трафик с базой данных.
 ...
 * >>>>> SQL -  это язык запросов с простыми-стандартными командами которые поддкрживаюся всеми типами баз данных;
 * >>>>> JPQL (Java Persistence query language) - это язык запросов такой же как SQL только (вместо имен и колонок таблиц базы данных) он использует имена классов Entity и их атрибуты;
