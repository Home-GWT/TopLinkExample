 ...
 *    Указывает что инструкции НЕмогут считывать данные которые были изменены другими транзакциями но еще не были зафиксированы.
 * 3. REPEATABLE READ  ( изоляция уровня повторяемого чтения )
 *    Указывает что инструкции НЕмогут считывать данные которые были изменены но еще не зафиксированы другими транзакциями.
 *    И другие транзакции НЕмогут изменять данные читаемые этой-текущей транзакцией до ее завершения.
 * SNAPSHOT
 * 4. SERIALIZABLE  ( упорядоченная изоляция )
 *    Указывает что инструкции НЕмогут считывать данные, которые были изменены другими транзакциями, но еще не были зафиксированы.
 *    И другие транзакции НЕмогут изменять данные, считываемые текущей транзакцией, до ее завершения.
 *    И другие транзакции НЕмогут вставлять новые строки со значениями ключа которые входят в диапазон ключей ДЛЯ считываемых инструкциями этой-текущей транзакции до ее завершения.
 */
