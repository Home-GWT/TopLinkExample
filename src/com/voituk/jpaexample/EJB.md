...
. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
Чтобы получить SSL-сертификат нужно сформировать запрос на выпуск сертификата (Certificate Signing Request).
После завершения веб-сервер создаст 2-а типа криптографических ключей:
- "приватный ключ"
- и "публичный ключ" (публичный ключ не является секретным и он помещается в запрос CSR)
...
++++++++++++++++++++++++++++++++++++
1. Генерируем (подпись .keystore - приватный ключ) свой приватный ключ
2. Генерируем/формируем (.crt - публичный ключ) запрос на выпуск сертификата Certificate Signing Request (CSR)
3. Отправляем CSR-файл (.crt - публичный ключ) в центр сертификации и получаем сертификат (.cer)
4. Импортируем сертификат (.cer) в хранилище (.keystore)
5. Подписываем jar-архив
6. Файл ".cer" отправляем всем кто хочет проверить архив
